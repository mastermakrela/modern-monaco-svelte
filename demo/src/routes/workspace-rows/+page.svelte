...
<section>
	<h2>Workspace from DB rows</h2>
	<p>
		Any array of <code>{'{ uuid, name, content }'}</code> objects — e.g. rows from a table — can back
		a workspace: <code>name</code> becomes the file path, <code>content</code> its text, and
		<code>uuid</code> tracks the row across renames. Edit a file in the editor or edit the JSON
		directly — both sides stay live.
	</p>
